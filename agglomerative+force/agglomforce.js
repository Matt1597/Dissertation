...
var nodes = []
    ...
    simulation.force('link', d3.forceLink().links(links).strength(.0015).distance(1));
    ...
    function layoutAndRenderData() {
      ...
      d3.csv("array.csv",function(data){
        ...
        nodes = d3.range(size).map(function() {
          ...
          var x = parseFloat(data[0][s])*3,
              y = parseFloat(data[1][s])*3,
              r = data[2][s]*3,
              l = parseInt(data[3][s]),
              clu = data[4][s],
...
//enter
      ...
      simulation.nodes(nodes)
        .force("collide", d3.forceCollide().strength(1).radius(function(d){ return d.radius; }).iterations(1));
});
});
}
//every tick
